...
function EmployeePourToi() {
  ...
  const recentFill = available.filter((v) => !weeklyPinnedIds.has(v.id) && new Date(v.createdAt || v.created_at) > sevenDaysAgo);
  const newThisWeek = [
    ...weeklyPinned,
    ...recentFill.sort((a, b) => new Date(b.createdAt || b.created_at).getTime() - new Date(a.createdAt || a.created_at).getTime()),
  ];
